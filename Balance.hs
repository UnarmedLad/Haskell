...
exercise6amax :: [(Weighing, [[Pattern]])] -> [(Weighing, Int)]
exercise6amax xs = map (\ (a,b) -> (a,maximum b)) $ map (\ (a,b) -> (a,map length b)) xs

exercise6alist :: Int -> [Pattern] -> [(Weighing, [[Pattern]])]
exercise6alist n xs = [(maybeGuess, [guessing maybeSecret maybeGuess xs | maybeSecret <- xs]) | maybeGuess <- makeList [L,R,Off] n] 
...
exercise6bentropy :: [(Weighing, [[Pattern]])] -> [(Weighing, Float)]
exercise6bentropy xs = map (\ (a,b) -> (a,sum $ map (\ x -> fromIntegral x * (log $ fromIntegral x)) $ b)) $ map (\ (a,b) -> (a,map length b)) $ map (\ (a,b) -> (a,filter (\ x -> x /= []) b)) xs
