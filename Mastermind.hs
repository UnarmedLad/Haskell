...
exercise2max :: [(Pattern, [[Pattern]])] -> [(Pattern, Int)]
exercise2max xs = map (\ (a,b) -> (a,maximum b)) $ map (\ (a,b) -> (a,map length b)) xs

exercise2list :: [Pattern] -> [(Pattern, [[Pattern]])]
exercise2list xs = [(maybeGuess, [guessing maybeSecret maybeGuess xs | maybeSecret <- xs]) | maybeGuess <- xs]
...
exercise3prep :: [(Pattern, [[Pattern]])] -> [(Pattern, Int)]
exercise3prep xs = map (\ (a,b) -> (a,length b)) $ map (\ (a,b) -> (a,filter (\ x -> x /= []) b)) xs
...
exercise4sum :: [(Pattern, [[Pattern]])] -> [(Pattern, Int)]
...
exercise5entropy :: [(Pattern, [[Pattern]])] -> [(Pattern, Float)]
exercise5entropy xs = map (\ (a,b) -> (a,sum $ map (\ x -> fromIntegral x * (logBase (fromIntegral $ length b) $ fromIntegral x)) $ b)) $ map (\ (a,b) -> (a,map length b)) $ map (\ (a,b) -> (a,filter (\ x -> x /= []) b)) xs
